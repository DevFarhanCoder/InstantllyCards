...
const generateTimeOptions = (): Array<
...
export default function Builder() {
  ...
  const createCardMutation = useMutation({
    mutationFn: async (payload: any) => {
      ...
      console.log(
        "📤 Builder: Sending card creation payload:",
        JSON.stringify({
      ...
      console.log(
        "✅ Builder: Card creation response:",
        JSON.stringify({
    ...
    onSuccess: async (_data, payload) => {
      ...
      try {
        ...
        console.warn(
          "Builder: failed to seed single-card cache after create",
          e,
        );
      }
      ...
      setTimeout(() => {
        ...
        try {
          console.log("Builder: raw create response:", _data);
        } catch (e) {}
        ...
        (async () => {
          try {
            ...
            console.log(
              "📱 Profile sync - personalPhone intentionally skipped to avoid conflicts",
            );
              ...
              try {
                try {
                  ...
                  try {
                    ...
                    console.warn(
                      "Builder: failed to merge user into AsyncStorage",
                      e,
                    );
                  }
                } catch (err: any) {
                  ...
                  if (
                    err?.status === 409 ||
                    /phone number already exists/i.test(msg)
                  ) {
                    console.warn(
                      "Builder: profile sync phone conflict, retrying without phone",
                    );
                    ...
                    try {
                      ...
                      console.log(
                        "Builder: profile sync response (without phone)",
                        resp2,
                      );
                      try {
                        const u = await AsyncStorage.getItem("user");
                        ...
                        console.warn(
                          "Builder: failed to merge user into AsyncStorage after retry",
                          e,
                        );
                      }
                    } catch (e2) {
                      console.warn(
                        "Builder: retry without phone also failed",
                        e2,
                      );
                    }
                  } else {
                ...
                try {
                  ...
                  if (createdCard) {
                    ...
                    console.log(
                      "Builder: saved created card to default_card (preview):",
                      createdCard._id ||
                        createdCard.id ||
                        JSON.stringify(createdCard).slice(0, 200),
                    );

                    // DO NOT add user's own cards to contacts-feed (home screen should only show other members' cards)
                    console.log(
                      "Builder: Skipping contacts-feed cache update for user's own card (should only show in My Cards)",
                    );
                    ...
                    try {
                      ...
                      console.warn(
                        "Builder: failed to invalidate queries after create",
                        e,
                      );
                    }
                  }
                } catch (e) {
                  console.warn("Builder: failed to save default_card", e);
                }
              } catch (e) {
            ...
            console.warn("Builder: unexpected error while syncing profile", e);
          }
        })(); // End of async IIFE
  ...
  const updateCardMutation = useMutation({
    mutationFn: async (payload: any) => {
      ...
      console.log(
        "📤 Builder: Sending card UPDATE payload:",
        JSON.stringify({
          birthdate: payload.birthdate,
          anniversary: payload.anniversary,
          name: payload.name,
    ...
    onSuccess: async (_data, payload) => {
        ...
        queryClient.setQueryData(["cards", currentUserId], (old: any) => {
          ...
          if (Array.isArray(old))
            return old.map((c: any) =>
              c && (c._id || c.id) === (updatedCard._id || updatedCard.id)
                ? updatedCard
                : c,
            );
          return old;
      ...
      setTimeout(() => {
        ...
        try {
          console.log(
            "Builder: raw update response:",
            JSON.stringify({
              birthdate: _data?.birthdate,
              anniversary: _data?.anniversary,
            }),
          );
        } catch (e) {}
        queryClient.invalidateQueries({ queryKey: ["cards", currentUserId] });
        ...
        (async () => {
          try {
            ...
            try {
              existingUserU = existingUserRawU
                ? JSON.parse(existingUserRawU)
                : null;
            } catch (e) {
              existingUserU = null;
            }
            ...
            console.log(
              "📱 Profile sync (update) - personalPhone intentionally skipped to avoid conflicts",
            );
            ...
            if (Object.keys(profileable).length > 0) {
              try {
                try {
                  ...
                  console.log(
                    "Builder update:onSuccess profile sync response",
                    resp,
                  );
                  try {
                    const u = await AsyncStorage.getItem("user");
                  ...
                  if (
                    ...
                    console.warn(
                      "Builder update:onSuccess: phone conflict, retrying without phone",
                    );
                    const reduced = { ...profileable };
                    delete reduced.phone;
                    try {
                      ...
                      console.log(
                        "Builder update:onSuccess profile sync response (without phone)",
                        resp2,
                      );
                      try {
                        const u = await AsyncStorage.getItem("user");
                      ...
                      console.warn(
                        "Builder update:onSuccess retry without phone failed",
                        e2,
                      );
                    }
                  } else {
                ...
                console.warn(
                  "Builder update:onSuccess failed to sync profile",
                  e,
                );
              }
            }
          } catch (e) {
            console.warn("Builder update:onSuccess unexpected", e);
          }
        })();
        ...
        (async () => {
          try {
            ...
            if (updatedCard) {
              ...
              console.log(
                "Builder: saved updated card to default_card (preview):",
                updatedCard._id ||
                  updatedCard.id ||
                  JSON.stringify(updatedCard).slice(0, 200),
              );
              ...
              try {
                ...
                console.warn(
                  "Builder: failed to refresh contacts-feed after update",
                  e,
                );
              }
            }
            ...
            console.warn(
              "Builder: failed to save default_card after update",
              e,
            );
          }
        })();
    ...
    onSuccess: () => {
      ...
      if (currentUserId) {
        queryClient.setQueryData(["cards", currentUserId], (old: any) => {
          if (!old) return [];
          if (Array.isArray(old))
            return old.filter((c: any) => (c._id || c.id) !== edit);
          return old;
        });
          ...
          {Array.from({ length: cells.length / 7 }).map((_, r) => (
            <View key={`row-${r}`} style={s.weekRow}>
              {cells.slice(r * 7, r * 7 + 7).map((day, ci) => {
                const isSelected =
                  day &&
                  selectedDay === `${year}-${pad(month + 1)}-${pad(day)}`;
                return (
                  <TouchableOpacity
                    key={`cell-${r}-${ci}-${day ?? "n"}`}
                    disabled={!day}
                  ...
                  >
                    <Text
                      style={[
                        s.dayText,
                        isSelected ? s.selectedDayText : undefined,
                      ]}
                    >
                      {day ?? ""}
                    </Text>
  ...
  const [companyPhones, setCompanyPhones] = useState<
    Array<{ countryCode: string; phone: string }>
  ...
  const [weeklySchedule, setWeeklySchedule] = useState<{
    [key: string]: { open: boolean; openTime: string; closeTime: string };
  }>({
  ...
  const [timePickerMode, setTimePickerMode] = useState<"open" | "close">(
    "open",
  );
  ...
  useEffect(() => {
    ...
    if (isEditMode && !formPopulated) {
      console.log(
        "⏸️ Skipping draft save - form not yet populated with existing card data",
      );
      return;
    }
  ...
  useEffect(() => {
    ...
    if (draftData) {
      ...
      if (isEditMode && existingCard && existingCard.updatedAt) {
        ...
        console.log(
          "🕐 Card updated at:",
          new Date(cardUpdateTime).toISOString(),
        );
        ...
        if (!shouldLoadDraft) {
          console.log(
            "⏭️ Draft is older than saved card - ignoring draft and loading from API",
          );
  ...
  useEffect(() => {
    ...
    if (
      card.companyPhones &&
      Array.isArray(card.companyPhones) &&
      card.companyPhones.length > 0
    ) {
      setCompanyPhones(
      ...
      setCompanyPhones([
        {
          countryCode: card.companyCountryCode || "91",
  ...
  useEffect(() => {
    if (companyPhoto) {
      console.log("📸 Company photo updated:", {
        type: companyPhoto.startsWith("data:")
          ? "Base64"
  ...
  const validate = () => {
    ...
    if (personalPhone && !/^\d{10}$/.test(personalPhone))
      e.personalPhone = "Enter 10 digits";
    if (companyEmail && !isEmail(companyEmail))
      e.companyEmail = "Invalid email";
    if (companyWebsite && !isURL(companyWebsite))
      e.companyWebsite = "Invalid URL";
    if (!isDigits(companyCountryCode)) e.companyCountryCode = "Only digits";
    if (companyPhone && !/^\d{10}$/.test(companyPhone))
      e.companyPhone = "Enter 10 digits";
    ...
    if (companyMapsLink && !isURL(companyMapsLink))
      e.companyMapsLink = "Invalid link";
  ...
  const pickBusinessPhoto = async () => {
    ...
    const res = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: (ImagePicker as any).MediaTypeOptions?.Images ??
        (ImagePicker as any).MediaType?.Images ?? ["Images"], // fixed property name
      base64: true,
      quality: 0.8,
    });
    console.log("📸 Image picker result:", {
      canceled: res.canceled,
      hasAssets: !!res.assets,
    });
  ...
  const onSave = async () => {
    ...
    if (isEditMode) {
      console.log(
        "🔄 Builder: UPDATE mode - payload preview:",
        JSON.stringify({
      ...
      console.log(
        "✨ Builder: CREATE mode - payload preview:",
        JSON.stringify({
          name: finalPayload.name,
          businessHours: finalPayload.businessHours,
          servicesOffered: finalPayload.servicesOffered,
    ...
    >
      <View style={s.sectionHeaderLeft}>
        <View
          style={[
            s.sectionIcon,
            sectionExpanded[section] && s.sectionIconActive,
          ]}
        >
          <Ionicons
            name={icon}
          ...
          >
            {sectionExpanded.personal && (
              <View style={s.sectionContent}>
                ...
                <View style={s.formField}>
                  <Text style={s.enhancedLabel}>
                    Birthdate<Text style={s.requiredIndicator}> *</Text>
                  </Text>
                    ...
                    <FormInput
                      ...
                      onChangeText={(t: string) => {
                        ...
                        if (parsed) {
                          ...
                          console.log(
                            "📅 Birthdate updated in real-time:",
                            parsed.iso,
                          );
                        } else if (formatted === "") {
                          // Clear if empty
                      ...
                      onBlur={() => {
                        ...
                        if (parsed) {
                          ...
                          setErrors((prev) => ({
                            ...prev,
                            birthdate: "Invalid date (dd-mm-yyyy)",
                          }));
                        } else {
                          setBirthdate(null);
                        }
                    ...
                    <FormInput
                      ...
                      onChangeText={(t: string) => {
                        ...
                        if (parsed) {
                          ...
                          console.log(
                            "📅 Anniversary updated in real-time:",
                            parsed.iso,
                          );
                        } else if (formatted === "") {
                          // Clear if empty
                      ...
                      onBlur={() => {
                        ...
                        if (parsed) {
                          ...
                          setErrors((prev) => ({
                            ...prev,
                            anniversary: "Invalid date (dd-mm-yyyy)",
                          }));
                        } else {
                          setAnniversary(null);
                        }
                ...
                <View style={s.formField}>
                  <PhoneInput
                    ...
                    onCountryCodeChange={(code) =>
                      setPersonalCountryCode(code.replace("+", ""))
                    }
              ...
              <View style={s.sectionContent}>
                ...
                <View style={s.photoContainer}>
                  ...
                  {companyPhoto && (
                    <Text style={{ fontSize: 10, color: "#666", marginTop: 4 }}>
                      Photo loaded:{" "}
                      {companyPhoto.startsWith("data:") ? "Base64" : "URL"} (
                      {companyPhoto.length} chars)
                    </Text>
                  )}
                  <View style={s.photoPreviewContainer}>
                    {companyPhoto ? (
                      <Image
                        ...
                        onLoad={() =>
                          console.log(
                            "✅ Business photo loaded successfully from:",
                            getImageUrl(companyPhoto),
                          )
                        }
                        onError={(e) => {
                          console.error(
                            "❌ Business photo load error:",
                            e.nativeEvent.error,
                          );
                          console.error(
                            "❌ Failed URL:",
                ...
                <View style={s.formField}>
                  ...
                  >
                    <Text
                      style={(() => {
                        const openDays = Object.entries(weeklySchedule).filter(
                          ([_, hours]) => hours.open,
                        );
                        return openDays.length > 0
                          ? s.servicesButtonTextFilled
                          : s.servicesButtonText;
                      })()}
                    >
                      {(() => {
                        const openDays = Object.entries(weeklySchedule).filter(
                    ...
                    <View style={s.customInputButtons}>
                      ...
                      <TouchableOpacity
                        style={[
                          ...
                          !customServiceInput.trim() &&
                            s.customInputAddButtonDisabled,
                        ]}
                    ...
                    return (
                      ...
                      >
                        <Text
                          style={[
                            s.serviceItemText,
                            isSelected && s.serviceItemTextSelected,
                          ]}
                        >
                          {item}
                        </Text>
                  ...
                  <View style={s.dayHeader}>
                    ...
                    <Pressable
                      onPress={() => {
                        ...
                        console.log(
                          `⏰ Toggling ${day}: ${weeklySchedule[day].open} -> ${newOpen}`,
                        );
