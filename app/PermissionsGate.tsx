...
export default function PermissionsGate() {
  ...
  async function syncContacts() {
    try {
      ...
      console.log(
        `📊 [PERMISSIONS-GATE] Found ${phoneNumbers.length} valid contacts`,
      );
      ...
      console.log(
        `📤 [PERMISSIONS-GATE] Syncing in ${totalBatches} batch(es)...`,
      );
      ...
      for (let i = 0; i < phoneNumbers.length; i += BATCH_SIZE) {
        ...
        console.log(
          `📤 [PERMISSIONS-GATE] Batch ${batchNumber}/${totalBatches} (${batch.length} contacts)`,
        );

        try {
          await api.post("/contacts/sync-all", { contacts: batch });
          console.log(
            `✅ [PERMISSIONS-GATE] Batch ${batchNumber}/${totalBatches} synced`,
          );
        } catch (batchError) {
          console.error(
            `❌ [PERMISSIONS-GATE] Batch ${batchNumber} failed:`,
            batchError,
          );
        }
      ...
      console.log(
        `✅ [PERMISSIONS-GATE] COMPLETE - ${phoneNumbers.length} contacts synced`,
      );
    } catch (syncError: any) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Auto-enter app after 3 seconds if user doesn't grant permissions
  ...
  return (
    <View style={s.container}>
      ...
      <TouchableOpacity
        disabled={!contactsAllowed || isSyncing}
        style={[
          s.enterBtn,
          { opacity: contactsAllowed && !isSyncing ? 1 : 0.4 },
        ]}
        onPress={enterApp}
      >
